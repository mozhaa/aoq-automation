...
from aoq_automation.telegram.filters import as_anime_query
...
async def process_url(url: str) -> bool:
    ...
    return await as_anime_query(message, state)


# NOTE: tests are invalid, because we need to mock db
# connection somehow (sqlite in-memory db probably)
