...
@pytest.fixture
def page_1():
    page = ShikiAnimeParser('https://shikimori.one/animes/y590-kage-kara-mamoru')
    with open(Path('.') / 'tests' / 'files' / 'shiki-kage-kara-mamoru.html', 'r', encoding='utf-8') as f:
        page.page = PyQuery(f.read())
    return page

def test_mal_id(page_1):
    assert page_1.mal_id == 590
    
def test_rating(page_1):
    assert f'{page_1.rating:.2f}' == '6.54'
...
def test_completed(page_1):
    assert page_1.completed== 729

def test_plan_to_watch(page_1):
    assert page_1.plan_to_watch == 719

def test_dropped(page_1):
    assert page_1.dropped == 79
...
def test_poster_url(page_1):
    assert page_1.poster_url == 'https://nyaa.shikimori.one/uploads/poster/animes/590/a062e49ec7e8dc7cd207a85dc9b3a991.jpeg'

def test_poster_thumbnail_url(page_1):
    assert page_1.poster_thumbnail_url == 'https://nyaa.shikimori.one/uploads/poster/animes/590/main_alt-5605f29ae19a25d17ea5d456a29ff57d.jpeg'
